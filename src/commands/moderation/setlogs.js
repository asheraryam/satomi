...
class SetLogs extends Command {
    ...
    handle ({ args, client, msg }, responder) {
        ...
        if (options === 'on') {
            ...
            .findOneAndUpdate({ serverID: msg.channel.guild.id }, { $set: { logChannel: channelID } }, { new: true }, (error, g) => {
                if (error) {
        ...
        if (options === 'off') {
            client.mongodb.models.guilds
            .findOneAndUpdate({ serverID: msg.channel.guild.id }, { $set: { logChannel: '' } }, { new: true }, (error, g) => {
                if (error) {
                    return responder.send(`${msg.author.mention} couldnt update the log channel`, { embed: {
                        color: client.redColor,
                        title: 'Set.LogChannel Error',
