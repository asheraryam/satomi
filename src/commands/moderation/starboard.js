...
class Starboard extends Command {
    ...
    handle ({ args, client, msg }, responder) {
        ...
        if (options === 'on') {
            ...
            .findOneAndUpdate({ serverID: msg.channel.guild.id }, { $set: { starChannel: channelID } }, { new: true }, (error, g) => {
                if (error) {
        ...
        if (options === 'off') {
            client.mongodb.models.guilds
            .findOneAndUpdate({ serverID: msg.channel.guild.id }, { $set: { starChannel: '' } }, { new: true }, (error, g) => {
                if (error) {
                    return responder.send(`${msg.author.mention} couldnt update the starboard channel`, { embed: {
                        color: client.redColor,
                        title: 'Set.StarChannel Error',
